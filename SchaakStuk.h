...
public:
    ...
    virtual void updateLocation(int r, int k){this->r = r; this->k = k;}
    pair<int, int> getLocation(){pair<int, int> location; location.first = r; location.second = k; return location;}
    virtual bool isKing() const {return false;}
    virtual string toShortString() const = 0;
    bool passant = false;
    bool isNotUsed() const {return moveCount == 0;}
    int moveCount = 0;
protected:
    int r, k;
private:
...
public:
    ...
    virtual string toString() const {return "Pion";}
